...
// fn world_to_view_normal(n: vec3<f32>) -> vec3<f32> {
//     let m4x4 = camera.view;
...
fn vs_main(
    ...
    // out.view_normal = world_to_view_normal(vin.normal);
    out.view_normal = normalize(vin.normal);
    return out;
}
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let k_diffuse = 0.7;
    let diffuse = max(dot(normalize(in.view_normal), light_dir), 0.0);
